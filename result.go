...
func parseTextRow(p []byte, f []Field) ([]interface{}, error) {
	...
	for i := range f {
		...
		isUnsigned = (int(f[i].Flag) & UNIQUE_FLAG) > 0

		switch f[i].Type {
		case MYSQL_TYPE_TINY, MYSQL_TYPE_SHORT, MYSQL_TYPE_INT24, MYSQL_TYPE_LONGLONG, MYSQL_TYPE_YEAR:
			if isUnsigned {
...
func parseBinaryRow(p []byte, f []Field) ([]interface{}, error) {
	...
	for i := range data {
		...
		case MYSQL_TYPE_SHORT, MYSQL_TYPE_YEAR:
			...
			pos += 2
			continue
		case MYSQL_TYPE_INT24, MYSQL_TYPE_LONGLONG:
			if isUnsigned {
...
type Resultset struct {
	...
	Fields     []Field
	FieldNames map[string]int // column name to Fields index
	Data       [][]interface{}
	RowPackets [][]byte
}
