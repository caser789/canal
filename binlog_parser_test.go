...
func TestBinlogParser_Parse(t *testing.T) {
	parser := NewBinlogParser()

	parser.format = &FormatDescriptionEvent{
		Version: 0x4,
		ServerVersion: []uint8{ // 50 bytes
			0x35, 0x2e, 0x36, 0x2e, 0x32, 0x30, 0x2d, 0x6c, 0x6f, 0x67, // 5.6.20-log
		...
		CreateTimestamp:   0x0,
		EventHeaderLength: 0x13, // 19
		EventTypeHeaderLengths: []uint8{
			0x38, 0xd, 0x0, 0x8, 0x0, 0x12, 0x0, 0x4,
			0x4, 0x4, 0x4, 0x12, 0x0, 0x0, 0x5c, 0x0,
			0x4, 0x1a, 0x8, 0x0, 0x0, 0x0, 0x8, 0x8,
			0x8, 0x2, 0x0, 0x0, 0x0, 0xa, 0xa, 0xa,
			0x19, 0x19, 0x0,
		}, // 每个 byte 代表一个 event 的 长度
	...
	parser.tables = map[uint64]*TableMapEvent{
		0x3043b: {
			...
			TableID:     0x3043b,
			Flags:       0x1,
			Schema:      []uint8{0x73, 0x65, 0x69, 0x75, 0x6d, 0x61, 0x73, 0x74, 0x65, 0x72}, // seiumaster
			Table:       []uint8{0x61, 0x70, 0x70, 0x5f, 0x63, 0x72, 0x6f, 0x6e},             // app_cron
			ColumnCount: 0x15,                                                                // 21
			ColumnType: []uint8{
				0x3, 0xf, 0xc, 0xc, 0xf, 0x3, 0xc, 0x3, 0xfc, 0xf, // MYSQL_TYPE_JSON...
				0x1, 0xfe, 0x2, 0xc, 0xf, 0xf, 0xc, 0xf, 0xf, 0x3,
				0xf,
			},
			ColumnMeta: []uint16{
				0x0, 0x180, 0x0, 0x0, 0x2fd, 0x0, 0x0, 0x0, 0x2, 0x180,
				0x0, 0xfe78, 0x0, 0x0, 0x180, 0x180, 0x0, 0x180, 0x180, 0x0,
				0x2fd,
			},
			NullBitmap: []uint8{0xf8, 0xfb, 0x17},
			// 11111000 11111011 10111
		},
	...
	b := &bytes.Buffer{}
	e.Dump(b)
	var expected = `=== WriteRowsEventV2 ===
Date: 2015-06-27 19:19:29
Log position: 247401479
Event size: 85
TableID: 198239
Flags: 1
Column count: 13
...
func TestBinlogParser_ParseEvent(t *testing.T) {
	tests := []struct {
		name      string
		byteData  []byte
		eventSize uint32
	...
	}{
		{
			...
			byteData: []byte{
				// 19 bytes header
				0x86, 0x4c, 0x9c, 0x5d, // 4 bytes timestamp
				0x03,
				0x65, 0x00, 0x00, 0x00, // 4 bytes server id
				0x13, 0x00, 0x00, 0x00, // 4 bytes event size (19 bytes)
				0x4d, 0x01, 0x00, 0x00, // 4 bytes log position
				0x00, 0x00, // 2 bytes flags
			},
			eventSize: uint32(19),
			s: `=== StopEvent ===
Date: 2019-10-08 16:44:54
Log position: 333
Event size: 19
Event data:
...
`,
		},
		{
			...
			byteData: []byte{
				// 19 bytes header
				0x15, 0x50, 0x9c, 0x5d, // 4 bytes timestamp
				0x03,
				0x65, 0x00, 0x00, 0x00, // 4 bytes server id
				0x17, 0x00, 0x00, 0x00, // 4 bytes event size (23 bytes)
				0x59, 0x01, 0x00, 0x00, // 4 bytes log position
				0x00, 0x00, // 2 byte4s flags

				0x7d, 0x82, 0xa8, 0x50, // CRC32 4 bytes
			},
			eventSize: uint32(23),
			s: `=== StopEvent ===
Date: 2019-10-08 17:00:05
Log position: 345
Event size: 23
Event data:
00000000  7d 82 a8 50                                       |}..P|
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			parser.format = &FormatDescriptionEvent{
				Version: 0x4,
				ServerVersion: []uint8{
					0x35, 0x2e, 0x36, 0x2e, 0x32, 0x30, 0x2d, 0x6c, 0x6f, 0x67,
					0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
					0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
					0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
					0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
				...
				EventTypeHeaderLengths: []uint8{
					0x38, 0xd, 0x0, 0x8, 0x0, 0x12, 0x0, 0x4, 0x4, 0x4,
					0x4, 0x12, 0x0, 0x0, 0x5c, 0x0, 0x4, 0x1a, 0x8, 0x0,
					0x0, 0x0, 0x8, 0x8, 0x8, 0x2, 0x0, 0x0, 0x0, 0xa,
					0xa, 0xa, 0x19, 0x19, 0x0,
				},
				ChecksumAlgorithm: 0x0,
			}

			r := bytes.NewReader(tt.byteData)
			_, err := parser.ParseSingleEvent(r, func(e *BinlogEvent) error {
				assert.Equal(t, STOP_EVENT, e.Header.EventType)
				assert.Equal(t, tt.eventSize, e.Header.EventSize)
				return nil
			})
			assert.NoError(t, err)

			e, err2 := parser.Parse(tt.byteData)
			assert.Equal(t, STOP_EVENT, e.Header.EventType)
			assert.Equal(t, tt.eventSize, e.Header.EventSize)
...
func TestBinlogParser_OnRow(t *testing.T) {
	testCases := []struct {
		byteData  []byte
		eventSize uint32
		eventType EventType
		...
		{
			[]byte{0x8d, 0x61, 0x72, 0x63, 0x13, 0xb, 0x0, 0x0, 0x0, 0x2c, 0x0, 0x0, 0x0, 0xa7, 0x0, 0x0, 0x0, 0x1, 0x0, 0x6c, 0x0, 0x0, 0x0, 0x0, 0x0, 0x1, 0x0, 0x2, 0x64, 0x62, 0x0, 0x3, 0x74, 0x62, 0x6c, 0x0, 0x1, 0x3, 0x0, 0x0, 0x63, 0x17, 0xe6, 0xf0},
			uint32(44),
			TABLE_MAP_EVENT,
		},
		// rows INT(1)
		{
			[]byte{0xb6, 0x61, 0x72, 0x63, 0x1e, 0xb, 0x0, 0x0, 0x0, 0x28, 0x0, 0x0, 0x0, 0xcf, 0x0, 0x0, 0x0, 0x1, 0x0, 0x6c, 0x0, 0x0, 0x0, 0x0, 0x0, 0x1, 0x0, 0x2, 0x0, 0x1, 0xff, 0x0, 0x1, 0x0, 0x0, 0x0, 0xf9, 0xf7, 0x89, 0x2a},
			uint32(40),
			WRITE_ROWS_EVENTv2,
		},
		// TABLE MAP EVENT tb(TINY)
		{
			[]byte{0x22, 0x6c, 0x72, 0x63, 0x13, 0xb, 0x0, 0x0, 0x0, 0x2e, 0x0, 0x0, 0x0, 0xfd, 0x0, 0x0, 0x0, 0x1, 0x0, 0x76, 0x0, 0x0, 0x0, 0x0, 0x0, 0x1, 0x0, 0x3, 0x64, 0x62, 0x31, 0x0, 0x4, 0x74, 0x62, 0x6c, 0x31, 0x0, 0x1, 0x1, 0x0, 0x0, 0x32, 0xec, 0x2f, 0x4},
			uint32(46),
			TABLE_MAP_EVENT,
		},
		// rows LONG(1)
		...
		{
			[]byte{0xeb, 0x64, 0x72, 0x63, 0x1e, 0xb, 0x0, 0x0, 0x0, 0x2d, 0x0, 0x0, 0x0, 0x2a, 0x1, 0x0, 0x0, 0x1, 0x0, 0x76, 0x0, 0x0, 0x0, 0x0, 0x0, 0x1, 0x0, 0x2, 0x0, 0x1, 0xff, 0x0, 0x1, 0x0, 0x0, 0x0, 0x0, 0x1, 0x0, 0x0, 0x0, 0x6e, 0xef, 0xb2, 0xb1},
			uint32(45),
			WRITE_ROWS_EVENTv2,
		},
	}
	...
	for _, tc := range testCases {
		...
		// e.Dump(os.Stdout)
		assert.NoError(t, err)
		assert.Equal(t, tc.eventType, e.Header.EventType)
		assert.Equal(t, tc.eventSize, e.Header.EventSize)
	}
}
